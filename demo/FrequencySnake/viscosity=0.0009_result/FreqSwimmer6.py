...
path = "_s=12_" + time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())
os.makedirs(dir + path)
shutil.copyfile("FreqSwimmer6.xml", dir + path + "/FreqSwimmer6.xml")
shutil.copyfile("FreqSwimmer6.py", dir + path + "/FreqSwimmer6.py")
# mkdirs
...
sim = MjSim(model)
A_skip, O_skip, L_skip = 10, 0.1, 5
for amplitude in np.arange(A_skip, 361, A_skip):  #
    for omega in np.arange(O_skip, 1.5, O_skip):  # 0.001 - 0.01
        for lamda in np.arange(L_skip, 181, L_skip):  # 10 - 180
            for y in np.arange(0.2, 1.01, 0.2):  # 0.1 - 0.5
                average_velocity, fp, _ = oneTest(model, False, False, 0, 0, 0, amplitude, omega, lamda, y)
