...
path = '/home/zheng/spinningup/data/paper_10.11/water/'\
       + time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()) + 'nocurriculum_s=0_'

logger_kwargs = dict(output_dir=path, exp_name='swimmer6'+time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()))

ppo_tf1(env_fn=env_fn, ac_kwargs=ac_kwargs, steps_per_epoch=3000, epochs=9000, save_freq=1000,
        target_kl=0.01, logger_kwargs=logger_kwargs, seed=10, pi_lr=3e-4)
